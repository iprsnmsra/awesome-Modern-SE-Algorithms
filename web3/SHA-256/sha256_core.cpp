...
public:
    static string hash(const string& message) {
        ...
        for (int i = 7; i >= 0; i--) {
            paddedMsg.push_back((bitLen >> (i * 8)) & 0xFF);
        }

        for (size_t i = 0; i < paddedMsg.size(); i += 64) {
            uint32_t w[64] = {0};
            for (int j = 0; j < 16; j++) {
                w[j] = (paddedMsg[i + j * 4] << 24) |
                       (paddedMsg[i + j * 4 + 1] << 16) |
                       (paddedMsg[i + j * 4 + 2] << 8) |
                       (paddedMsg[i + j * 4 + 3]);
            }
...
int main() {
    ...
    assert(customHash == expectedHash);

    cout << "C++ SHA-256 Engine Test Passed! Mathematical Bit-Scrambling Verified.\n";
    return 0;
}